...
VAULT_ADDR="$(curl -Ssf ${CONSUL_HTTP_ADDR}/v1/kv/platform/conf/sandbox/global/vault_addr?raw)"
...
vault_sealed="$(curl -Ss --connect-timeout 4 \
  "${VAULT_SERVERS[0]}/v1/sys/seal-status" \
  | jq -r .sealed)"

if [[ "${vault_init}" == "false" ]] || \
   [[ "${vault_sealed}" == "true" ]]; then
  vault_reset
fi
