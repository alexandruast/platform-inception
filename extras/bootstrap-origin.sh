...
JENKINS_BUILD_JOB=infra-generic-nomad-server-deploy JENKINS_ADDR=http://127.0.0.1:${tunnel_port} ANSIBLE_EXTRAVARS="{}" ANSIBLE_TARGET=${server_nodes} ANSIBLE_EXTRAVARS="{'serial_value':'100%','service_bind_ip':'{{ansible_host}}'}" ./jenkins-query.sh ./common/jobs/build-infra-generic-deploy-job.groovy
...
JENKINS_BUILD_JOB=infra-generic-vault-server-deploy JENKINS_ADDR=http://127.0.0.1:${tunnel_port} ANSIBLE_EXTRAVARS="{}" ANSIBLE_TARGET=${server1_ip} ./jenkins-query.sh ./common/jobs/build-infra-generic-deploy-job.groovy
