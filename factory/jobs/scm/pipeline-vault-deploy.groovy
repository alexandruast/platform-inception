  ...
  stage('prepare') {
    checkout([$class: 'GitSCM', 
      ...
      sh '''
        ...
        curl -Ss --request PUT --data "${SSH_TARGET}" http://127.0.0.1:8500/v1/kv/jenkins/pipeline_vault_deploy_ssh_target
      '''
  }
  stage('provision') {
    sh '''#!/usr/bin/env bash
      ...
      SSH_TARGET="$(curl -Ss http://127.0.0.1:8500/v1/kv/jenkins/pipeline_vault_deploy_ssh_target?raw)"
